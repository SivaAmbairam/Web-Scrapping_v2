    ...
    while threshold >= 0:
        ...
        if prev_threshold is None or threshold != prev_threshold:  # Check if the threshold has changed
            ...
            with open(output_file, 'w', newline='', encoding='utf-8') as master_file:
                ...
                # Loop through each Flinn product and find the best match from Nasco's products
                for original_flinn_row, flinn_word_set in flinn_products:
                    ...
                    for original_nasco_row, nasco_word_set in nasco_products:
                        similarity = word_similarity(flinn_word_set, nasco_word_set)
                    ...
                    nasco_colors = [color for color in color_names if best_match and re.search(rf'\b{color}\b', best_match['Nasco_product_name'], re.IGNORECASE)]
                    ...
                    nasco_ml_mm = re.findall(r'\b\d+(\.\d+)?\s*(mL|mm)\b', best_match['Nasco_product_name'], re.IGNORECASE) if best_match else []

                    if best_match_score >= threshold:
                        if set(flinn_colors) == set(nasco_colors) and set(flinn_ml_mm) == set(nasco_ml_mm):
                            writer.writerow([original_flinn_row['Flinn_product_category'], original_flinn_row['Flinn_product_sub_category'], original_flinn_row['Flinn_product_id'], original_flinn_product, original_flinn_row['Flinn_product_quantity'], original_flinn_row['Flinn_product_price'], original_flinn_row['Flinn_product_url'], best_match['Nasco_product_category'], best_match['Nasco_product_sub_category'], best_match['Nasco_product_id'], best_match['Nasco_product_name'], best_match['Nasco_product_quantity'], best_match['Nasco_product_price'], best_match['Nasco_product_url'], best_match_score])
            ...
            # Get the unmatched Nasco products for the next iteration
            with open('Nasco_products.csv', 'r', encoding='utf-8') as nasco_file:
...
with open(final_output_file, 'w', newline='') as final_file:
    writer = csv.writer(final_file)
    writer.writerow(['Flinn_product_category', 'Flinn_product_sub_category', 'Flinn_product_id', 'Flinn_product_name', 'Flinn_product_quantity', 'Flinn_product_price', 'Flinn_product_url', 'Nasco_product_category', 'Nasco_product_sub_category', 'Nasco_product_id', 'Nasco_product_name', 'Nasco_product_quantity', 'Nasco_product_price', 'Nasco_product_url', 'Match_Score'])
    for match in matched_products:
        writer.writerow([match[0]['Flinn_product_category'], match[0]['Flinn_product_sub_category'], match[0]['Flinn_product_id'], match[0]['Flinn_product_name'], match[0]['Flinn_product_quantity'], match[0]['Flinn_product_price'], match[0]['Flinn_product_url'], match[1]['Nasco_product_category'], match[1]['Nasco_product_sub_category'], match[1]['Nasco_product_id'], match[1]['Nasco_product_name'], match[1]['Nasco_product_quantity'], match[1]['Nasco_product_price'], match[1]['Nasco_product_url'], match[2]])
