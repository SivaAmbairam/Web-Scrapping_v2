    ...
    while threshold >= 0:
        ...
        if prev_threshold is None or threshold != prev_threshold:  # Check if the threshold has changed
            ...
            with open(output_file, 'w', newline='', encoding='utf-8') as master_file:
                ...
                # Write headers
                writer.writerow(['Flinn_product_category', 'Flinn_product_sub_category', 'Flinn_product_id', 'Flinn_product_name', 'Flinn_product_quantity', 'Flinn_product_price', 'Flinn_product_url', 'Frey_product_category', 'Frey_product_sub_category', 'Frey_product_id', 'Frey_product_name', 'Frey_product_quantity', 'Frey_product_price', 'Frey_product_url', 'Match_Score'])

                # Loop through each Flinn product and find the best match from Frey's products
                for original_flinn_row, flinn_word_set in flinn_products:
                    ...
                    for original_frey_row, frey_word_set in frey_products:
                        similarity = word_similarity(flinn_word_set, frey_word_set)
                    ...
                    if best_match_score >= threshold:
                        if set(flinn_colors) == set(frey_colors) and set(flinn_ml_mm) == set(frey_ml_mm):
                            writer.writerow([original_flinn_row['Flinn_product_category'], original_flinn_row['Flinn_product_sub_category'], original_flinn_row['Flinn_product_id'], original_flinn_product, original_flinn_row['Flinn_product_quantity'], original_flinn_row['Flinn_product_price'], original_flinn_row['Flinn_product_url'], best_match['Frey_product_category'], best_match['Frey_product_sub_category'], best_match['Frey_product_id'], best_match['Frey_product_name'], best_match['Frey_product_quantity'], best_match['Frey_product_price'], best_match['Frey_product_url'], best_match_score])
            ...
            # Get the unmatched Frey products for the next iteration
            with open('Frey_products.csv', 'r', encoding='utf-8') as frey_file:
...
with open('Flinn_Products.csv', 'r', encoding='utf-8') as flinn_file, open('Frey_products.csv', 'r', encoding='utf-8') as frey_file:
...
with open(final_output_file, 'w', newline='', encoding='utf-8') as final_file:
    ...
    for match in matched_products:
        writer.writerow([match[0]['Flinn_product_category'], match[0]['Flinn_product_sub_category'], match[0]['Flinn_product_id'], match[0]['Flinn_product_name'], match[0]['Flinn_product_quantities'], match[0]['Flinn_product_prices'], match[0]['Flinn_product_urls'], match[1]['Frey_product_category'], match[1]['Frey_product_sub_category'], match[1]['Frey_product_id'], match[1]['Frey_product_name'], match[1]['Frey_product_quantity'], match[1]['Frey_product_price'], match[1]['Frey_product_url'], match[2]])
